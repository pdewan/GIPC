...
//		if (!anEnumClass.isEnum()) return null;
	...
	public Object parseBooleanOrEnum(Class aClass, String aValue){
		if (aClass == Boolean.TYPE)
			return parseBoolean(aValue);
		if (aClass.isEnum())
			return parseEnum(aClass, aValue);
		System.err.println(aClass.getSimpleName() + " is not bool or an enum class");
		return null;
	}
	...
	public Object[] parseMethodArgs(Method aMethod, String[] aTokens) {
		...
		for (int i = 0; i < aParameterTypes.length; i++) {
			result[i] = parseBooleanOrEnum(aParameterTypes[i], aTokens[i+1]);
		}
		return result;
	}
	@Override
