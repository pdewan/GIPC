...
/**
 * A class thatmMake the selector thread invoke remote calls through a separate
 * thread - the asynchronous threading approach to handling incoming procedure
 * calls.
 *
 */
...
//		extends ADuplexReceivedCallInvoker 
	...
	static BlockingQueue<MessageWithSourceAndReceiver> callQueue = new LinkedBlockingQueue(
			AScatterGatherSelectionManager.getMaxOutstandingWrites());
...
//		super(aLocalRemoteReferenceTranslator, aReplier, theRPCRegistry);
	...
	public void messageReceived(String aSender, Object aMessage) {
		if (callQueue.size() == AScatterGatherSelectionManager.getMaxOutstandingWrites()) {
			Tracer.error("max outstanding method calls exceed limit of"
					+ AScatterGatherSelectionManager.getMaxOutstandingWrites());
		} else {
//			System.out.println ("Call Queue added:" + aMessage);
			MessageWithSourceAndReceiver aMessageWithSourceAndReceiver = new AMessageWithSourceAndReceiver(aSender,
					aMessage, synchronousReceivedCallInvoker);
...
//			Tracer.setKeywordPrintStatus(this, true);
				...
				ReceivedCallEndedOld.newCase(this, message.getReceiveListener(), message.getSource(),
						(RemoteCall) message.getMessage());
			}
